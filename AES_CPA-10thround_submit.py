...
print((key_guess == known_keys[0]).all())
...
for subkey in range(16):
    ...
    for guessed_key in range(256):
        sns.scatterplot(x=range(400), y=crvs[subkey, guessed_key ,400:800])

        plt.title(f"Sub Keys {subkey}")
    if subkey == 15:
        plt.xlabel("Time")
...
sns.scatterplot(x=range(256), y=crvs[selected_subkey, :, :].max(axis=1))

for key in range(256):
    max_crv = crvs[selected_subkey, :, :].max()
    if crvs[selected_subkey, key, :].max() == max_crv:
        plt.text(x=key + 1, y=max_crv + 0.0001, s=str(key))
...
fig.suptitle("subkey vs correlation values", fontsize=18, y=0.95)

for _, ax in enumerate(axs):
    c = next(colors)["color"]
    ax.scatter(range(256), crvs[_,:,:].max(axis=1), color=c, alpha=1)
    _crvs = crvs[_,:,:].max(axis=1)
    for i in range(256):
        max_corr = _crvs.max()
        if _crvs[i] == max_corr:
            ax.text(x=i + 1, y=max_corr - 0.001, s=str(i))
